...
class Student(AbstractUser):
    code = models.CharField(max_length=10, unique=True,validators=[
        RegexValidator(
            r'^\d{10}$', 
            'El código deber ser un numero de max 10 dígitos')])
...
class Teacher(models.Model):
    ...
    dni = models.CharField(max_length=10, unique=True, validators=[
        RegexValidator(
            r'^\d{10}$', 
            'La cédula deber ser un numero de max 10 dígitos')])
